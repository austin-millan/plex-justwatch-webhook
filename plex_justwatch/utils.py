...
class PlexJustWatchPlaylistManager:
    ...
    def formatProviderPlaylistName(self, mediaType, providerName):
    ...
    def providerHasTitle(self, title, providerURL, searchResults):
    ...
    def syncLibrary(self, library=''):
        ...
        for media_item in self.plex.library.section(library).search():
            providers = self.getCurrentJustwatchProvidersForTitle(media_item.title)
            if providers:
    ...
    def getCurrentJustwatchProvidersForTitle(self, title):
        ...
        for provider_name in getAllProviderNames():
            ...
            hasMedia = self.providerHasTitle(title=title, providerURL=provider_url, searchResults=results)
            if hasMedia:
    ...
    def processEvent(self, payload):
        ...
        if event_type == "library.new":
            ...
            self.pruneTitleFromPlaylists(title)
            if self.update_user_playlists:
                self.copyProviderPlaylistsToUsers()

    def getProviderPlaylist(self, mediaType, providerName):
        ...
        formattedPlaylistName = self.formatProviderPlaylistName(mediaType, providerName)
        for playlist in playlists:
            if playlist.title == formattedPlaylistName:
                return playlist
        return None

    def pruneTitleFromPlaylists(self, title):
    ...
    def updateTitleInPlaylist(self, providerName, title, library='Movies'):
        ...
        for item in items:
            ...
            if item.type == 'movie':
                mediaType == 'Movies'
            formattedPlaylistName = self.formatProviderPlaylistName(mediaType, providerName)
            provider_playlist = self.getProviderPlaylist(mediaType, providerName)
    ...
    def updateMediaItemInPlaylist(self, providerName, media_item):
        if providerName == "" or providerName == None:
            self.logger.error("Cannot add item to empty provider.")
            return
        if media_item == None:
        ...
        formattedPlaylistName = self.formatProviderPlaylistName(media_type, providerName)
        provider_playlist = self.getProviderPlaylist(media_type, providerName)
        if provider_playlist == None:
            self.logger.info(f"Creating playlist \"{formattedPlaylistName}\"")
            provider_playlist = self.plex.createPlaylist(formattedPlaylistName, item)
    ...
    def copyProviderPlaylistsToUsers(self):
        ...
        for playlist in all_playlists:
            for provider in getAllProviderNames():
                for mediaType in ['Movies', 'Shows']:
                    formattedTitle = self.formatProviderPlaylistName(mediaType, provider)
...
if __name__ == '__main__':
    ...
    manager.processEvent(NEW_CONTENT_PAYLOAD)
    # manager.getCurrentPlexProviderPlaylists()
